...
class FaqCategoryController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($id) {
            ...
            save_log('faq_category', 'id', $id, $input, 'Update');
            $update = \DB::table('faq_category')->where('id', $id)->update($input);
            if ($update) {
            ...
            $input['postby'] = $postby;
            if ($data = FaqCategory::forceCreate($input)) {
    ...
    public function faqcategory_list(Request $request)
    {
        $date_type = get_constant_value('DEFAULT_DATEPICKER');

        $data = FaqCategory::get_faqcategory_list();
        // echo "<pre>";
        // print_r($data);
        // die();
        ...
        foreach ($data as $key => $row) {
            ...
            $array[$i]['action'] = '<a href="javascript:void(0)" class="btnEdit" data-url="/api/faq_category/faqcategory_edit" data-id=' . $row->id . ' data-targetForm="faqcatForm" data-edittype="template"><i class="fa fa-edit"></i></a>
            &nbsp
            <a href="javascript:void(0)" class="btnDelete" data-url="/api/faq_category/faqcategory_delete" data-id=' . $row->id . ' data-targetForm="pageForm" data-edittype="template"><i class="fa fa-trash"></i></a>
            ';
    ...
    public function faqcategory_edit(Request $request)
    {

        $id = $request->get('id');
        $data = \DB::table('faq_category')->where('id', $id)->first();
        $view = view("Faq/FaqCategory")
            ->with('data', $data);
    ...
    public function faqcategory_delete(Request $request)
    {
        ...
        $id = $request->get('id');
        save_log('faq_category', 'id', $id, false, 'Delete');
