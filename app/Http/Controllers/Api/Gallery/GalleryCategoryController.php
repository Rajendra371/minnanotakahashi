...
public function store(Request $request)
    {
       ...
       if ($validator->fails()) {
        ...
        $input['modifytime']=date('H:i:s');
         save_log('gallery_categories','id',$id,$input,'Update');
         $update=\DB::table('gallery_categories')->where('id',$id)->update($input);
         if($update)
         {
        ...
        else
        {
            ...
            $input['postby']=$postby;
          if($data=GalleryCategory:: forceCreate($input)){
    ...
    public function gallerycategory_list(Request $request){
        $date_type=get_constant_value('DEFAULT_DATEPICKER');

        $data = GalleryCategory::get_gallerycategory_list();
        // echo "<pre>";
        // print_r($data);
        // die();
        ...
        foreach($data as $key=>$row){
            ...
            $array[$i]['action'] = '<a href="javascript:void(0)" class="btnEdit" data-url="/api/gallery_cat_setup/gallerycategory_edit" data-id='.$row->id.' data-targetForm="gallerycatForm" data-edittype="template"><i class="fa fa-edit"></i></a>
            &nbsp
            <a href="javascript:void(0)" class="btnDelete" data-url="/api/gallery_cat_setup/gallerycategory_delete" data-id='.$row->id.' data-targetForm="gallerycatForm" data-edittype="template"><i class="fa fa-trash"></i></a>
            ';
     ...
     public function gallerycategory_edit(Request $request)
    {

        $id=$request->get('id');
        $data = \DB::table('gallery_categories')->where('id', $id)->first();
        $view = view("Gallery/GalleryCategory")
        ->with('data',$data);
    ...
    public function gallerycategory_delete(Request $request)
    {
        ...
        $id=$request->get('id');
          save_log('gallery_categories','id',$id,false,'Delete');
