...
use App\Models\NNE\NneCategory;
...
class NneCategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($id) {
            ...
            $data = NneCategory::where('id', $id)->first();
            ...
            save_log('nne_category', 'id', $id, $input, 'Update');
            $update = \DB::table('nne_category')->where('id', $id)->update($input);
            if ($update) {
            ...
            $input['postby'] = $postby;
            if ($data = NneCategory::forceCreate($input)) {
    ...
    public function nnecategory_list(Request $request)
    {
        $date_type = get_constant_value('DEFAULT_DATEPICKER');

        $data = NneCategory::get_nnecategory_list();
        // echo "<pre>";
        // print_r($data);
        // die();
        ...
        foreach ($data as $key => $row) {
            ...
            $array[$i]['action'] = '<a href="javascript:void(0)" class="btnEdit" data-url="/api/nne_category/nnecategory_edit" data-id=' . $row->id . ' data-targetForm="nnecForm" data-edittype="template"><i class="fa fa-edit"></i></a>
            &nbsp
            <a href="javascript:void(0)" class="btnDelete" data-url="/api/nne_category/nnecategory_delete" data-id=' . $row->id . ' data-targetForm="nnecForm" data-edittype="template"><i class="fa fa-trash"></i></a>
            ';
    ...
    public function nnecategory_edit(Request $request)
    {

        $id = $request->get('id');
        // $location =\DB::table('advertisement_location')->where(['is_active'=>'Y'])->get();
        $data = \DB::table('nne_category')->where('id', $id)->first();
        $view = view("NNE/NneCategory")
            ->with('data', $data);
    ...
    public function nnecategory_delete(Request $request)
    {
        ...
        $id = $request->get('id');
        save_log('nne_category', 'id', $id, false, 'Delete');
        $data = NneCategory::where('id', $id)->delete();
        if ($data) {
