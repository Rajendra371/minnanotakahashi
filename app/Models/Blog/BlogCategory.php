...
class BlogCategory extends Model
{
    ...
    public static function get_blogcategory_list(){
        $get = $_GET;
        ...
        $nquery = \DB::table('blog_category as ecl')
        ...
        if(!empty($get['sSearch_1'])){
            $nquery->where('description','like',"%".$get['sSearch_1']."%");
        }
       

    

        $query = \DB::table('blog_category as ecl')
        // ->leftjoin('employee as e','e.device_id','=','ecl.empid')
        ->select('*');
        ...
        if(!empty($get['sSearch_1'])){
            $query->where('description','like',"%".$get['sSearch_1']."%");
        }
    ...
    public static function get_all_blogcategory_data($id=false){
