...
class NneSetup extends Model
{
    ...
    public static function get_nne_list(){
        $get = $_GET;
        ...
        $nquery = \DB::table('nne as ecl')
        ->leftjoin('nne_category as e','e.id','=','ecl.category_typeid')
        ...
        if(!empty($get['sSearch_3'])){
            $nquery->where('ecl.startdate','like',"%".$get['sSearch_3']."%");
        }
        if(!empty($get['sSearch_4'])){
            $nquery->where('ecl.enddate','like',"%".$get['sSearch_4']."%");
        }

    

        $query = \DB::table('nne as ecl')
        ->leftjoin('nne_category as e','e.id','=','ecl.category_typeid')
        ->select('ecl.*','e.category_name');
        ...
        if(!empty($get['sSearch_3'])){
            $query->where('ecl.startdate','like',"%".$get['sSearch_3']."%");
        }
        if(!empty($get['sSearch_4'])){
            $query->where('ecl.enddate','like',"%".$get['sSearch_4']."%");
        }
        $order_by = '';
        $order = '';
        if($get['sSortDir_0']){
    ...
    public static function get_all_nne_data($id=false){
        $data=DB::table('nne as ecl')
        ->leftjoin('nne_category as e','e.id','=','ecl.category_typeid')
        ->select('ecl.*','e.category_name')
