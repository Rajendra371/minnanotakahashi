...
class Ourproduct extends Model
{
    ...
    public static function get_ourproduct_list(){
        $get = $_GET;
        ...
        if(!empty($get['sSearch_3'])){
            $nquery->where('short_description','like',"%".$get['sSearch_3']."%");
        }
        if(!empty($get['sSearch_4'])){
            $nquery->where('description','like',"%".$get['sSearch_4']."%");
        }
        ...
        if(!empty($get['sSearch_3'])){
            $query->where('short_description','like',"%".$get['sSearch_3']."%");
        }
        if(!empty($get['sSearch_4'])){
            $query->where('description','like',"%".$get['sSearch_4']."%");
        }
    ...
    public static function get_all_ourproduct_data($id=false){
    ...
    public static function get_ourproductcategory_list(){
        $get = $_GET;
        ...
        $nquery = \DB::table('ourproduct_category as ecl')
        // ->leftjoin('employee as e','e.device_id','=','ecl.empid')
        ->select('*');
        ...
        if(!empty($get['sSearch_1'])){
            $nquery->where('ourproduct_cat','like',"%".$get['sSearch_1']."%");
        }
        ...
        $query = \DB::table('ourproduct_category as ecl')
        // ->leftjoin('employee as e','e.device_id','=','ecl.empid')
        ->select('*');
        
        if(!empty($get['sSearch_1'])){
            $query->where('ourproduct_cat','like',"%".$get['sSearch_1']."%");
        }
        if(!empty($get['sSearch_2'])){
    ...
    public static function get_all_ourproductcategory_data($id=false){
