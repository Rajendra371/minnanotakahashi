...
class PortfolioCategory extends Model
{
    ...
    public static function get_portfoliocategory_list(){
        $get = $_GET;
        ...
        $nquery = \DB::table('portfolio_category as ecl')
        ...
        if(!empty($get['sSearch_2'])){
            $nquery->where('url','like',"%".$get['sSearch_2']."%");
        }
        if(!empty($get['sSearch_3'])){
            $nquery->where('content','like',"%".$get['sSearch_3']."%");
        }
       

    

        $query = \DB::table('portfolio_category as ecl')
        // ->leftjoin('employee as e','e.device_id','=','ecl.empid')
        ->select('*');
        ...
        if(!empty($get['sSearch_2'])){
            $query->where('url','like',"%".$get['sSearch_2']."%");
        }
        if(!empty($get['sSearch_3'])){
            $query->where('content','like',"%".$get['sSearch_3']."%");
        }
