...
<div class="table">
	...
	<div class="bottom-header">
		<p style="font-weight: bold;text-align: right;font-size: 16px;">
			V. No.:{{$data->voucher_no}}
		</p>
		<p style="text-align: right;font-size: 16px;">
			Date:{{$data->paymentdatead}}
		</p>
		<p style="text-align: left;font-size: 16px;">
			Pay To:{{$data->supplier_name.' '.$data->supplier_address}}
		</p>
	</div>
	<table style="border: 1px solid;width: 100%;" cellspacing="0">
		<thead>
			...
			<tr>
				...
				<td style="border-bottom: 1px solid;border-right: 1px solid;padding: 21px 0;">Advance Payment</td>
				<td style="border-bottom: 1px solid;border-right: 1px solid;padding: 21px 0;">0.</td>
				<td style="border-bottom: 1px solid;padding: 21px 0;">{{$data->amount}}</td>
			</tr>		
			<tr>
			...
			<tr>
				...
				<td style="border-right: 1px solid;padding: 21px 0;"></td>
				<td style="padding: 21px 0;">{{number_format($data->amount,2)}}</td>
            </tr>
            <tr>
                <td colspan="4">
                    In Word :{{convert_to_word($data->amount)}}
                </td>
            </tr>
		</thead>
	</table>
	...
	<div class="footer">
        ...
        <p style="float: left;">Bank Account:@if(!empty($data)) {{$data->bank_name.' '.$data->bank_account_no}} @endif</p>
        <p style="float: right;word-spacing: 2px;position: absolute;right: 173px;">Cheque No:{{$data->chequeno}}</p>
		</div>
		<table style="width: 100%;page-break-after: avoid;"cellspacing="0">
			<tr>
				...
				<td style="font-size: 18px;width:30%;    padding: 21px 0;">Date:{{$data->paymentdatead}}</td>
			...
			<tr>
            <td style="font-size: 18px;width:33%;    padding: 21px 0;">Received By:{{$data->receiver_name.' '.$data->receiver_contactno}}</td>
				<td style="font-size: 18px;width:30%;    padding: 21px 0;">Signature:___________________________________</td>
            <td style="font-size: 18px;width:30%;    padding: 21px 0;">Date:{{$data->paymentdatead}}</td>
