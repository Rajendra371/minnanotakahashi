<div class="row">
    <input type="hidden" name="id" value="{{!empty($data->id)?$data->id:''}}">
    <div class="col-md-6 mb-2 form-group"   >
        <label>Technology Category<code>*</code>: </label>
        <select name="technology_catid" id="technology_catid" class="form-control">
            <option>--Select--</option>
            @if(!empty($category)) 
            @foreach($category as $cat)
                <option value="{{$cat->id}}" @if($data->technology_catid==$cat->id) {{"selected=selected"}} @endif  >{{$cat->cat_name}} </option>
            @endforeach
            @endif
        </select>
    </div> 

    <div class="col-md-6 mb-2 form-group">
        <label>Code<code>*</code>: </label>
        <input name="code" id="code" placeholder="Technology Code" class="form-control" value="{{!empty($data->code)?$data->code:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>Technology Title<code>*</code>: </label>
        <input name="title" id="title" placeholder="Technology Title" class="form-control" value="{{!empty($data->title)?$data->title:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>Slug: </label>
        <input name="slug" id="slug" class="form-control" value="{{!empty($data->slug)?$data->slug:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>Icon<code>*</code>: </label>
        <input name="icon" id="icon" class="form-control" value="{{!empty($data->icon)?$data->icon:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>Icon Type<code>*</code>: </label>
        <select name="icon_type" id="icon_type" class="form-control">
            <option value="icon" @if($data->icon_type=='ICON') {{"selected=selected"}} @endif >Icon</option>
            <option value="image" @if($data->icon_type=='IMAGE') {{"selected=selected"}} @endif >Image</option>
        </select>
        {{-- <input name="icon_type" id="icon_type" class="form-control" value="{{!empty($data->icon_type)?$data->icon_type:''}}"></input> --}}
    </div>

    <div class="col-12 form-group">
        <label class="">Image<code>*</code>:</label>
        <div class="file-upload-wrapper" data-text="Select your image!">
        <input name="file" type="file" class="file-upload-field form-control-file"></div>
        @if(!empty($data->image))
        <input type="hidden" name="old_img_file" value="{{$data->image}}" >
        <img src="{{asset('uploads/technology_image/'.$data->image)}}" style="height:150px; width:150px">
        @endif
    </div>

    <div class="col-md-12 mb-2 form-group">
        <label>Short Description<code>*</code>: </label>
        <textarea  name="short_description" id="short_description" class="form-control">{{!empty($data->short_description)?$data->short_description:''}}</textarea>
    </div>

    <div class="col-md-12 mb-2 form-group">
        <label>Content<code>*</code>: </label>
        <textarea  name="description" id="description" class="form-control ckeditor">{{!empty($data->description)?$data->description:''}}</textarea>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>SEO Title: </label>
        <input name="seo_title" id="seo_title" class="form-control" placeholder="SEO Title" value="{{!empty($data->seo_title)?$data->seo_title:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>SEO Keyword: </label>
        <input name="seo_keyword" id="seo_keyword" class="form-control" placeholder="SEO Keyword" value="{{!empty($data->seo_keyword)?$data->seo_keyword:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>SEO Description: </label>
        <input name="seo_description" id="seo_description" placeholder="SEO Description" class="form-control " value="{{!empty($data->seo_description)?$data->seo_description:''}}"></input>
    </div>

    <div class="col-md-6 mb-2 form-group">
        <label>Order: </label>
        <input type="number" name="order" id="order" placeholder="Order" class="form-control" value="{{!empty($data->order)?$data->order:''}}"></input>
    </div>
    
    <div class="col-md-6">
        <div class="form-group">            
            <div class="checkbox">
            <input type="checkbox" id="is_publish" name="is_publish" value="Y" {{$data->is_publish == 'Y' ? 'checked':''}} />
            <label></label>
            <span>Is Publish</span>
            </div>
        </div>
            
    </div>
                    
    <div class="col-md-12">
        <div class="float-right">
            <button type="submit" class="save btn btn-primary btn-md" data-targetdiv='education_info_employeediv' data-redirect_type='form' data-is_table_refresh="Y" data-target_btn="btnrefresh_edu" > <i class="fa fa-dot-circle-o mr-1"></i>Update</button>
            <button type="button" class="btnreset btn btn-danger btn-md"><i class="fa fa-ban mr-1"></i>Reset</button>  
        </div>
    </div>
    <div class="alert-success success"></div>
    <div class="alert-danger error"></div>

</div>
<script>
load_datepicker('N');
load_ckeditor();
</script>